...
- 数据总线请求寄存器现在对地址、写数据和读写使能全部给出确定复位值，避免同一
  时序块中部分寄存器有复位、部分寄存器无复位；
- 延迟 load 响应只保存实际使用的两位字节偏移，不再保存随后被裁剪的 32 位
  `alu_c_r`；
- `ic_cpu_raddr[1:0]` 在 AXI Master 内按 32 位单拍事务对齐为 `2'b00`。低位被
  综合器裁剪属于接口对齐的预期结果，不是地址丢失。

上述修改保持 AXI 请求、响应和 Trace 层级不变。需要在下次 Vivado 重建时确认
第一项和第三项旧警告已消失；地址低位提示即使仍出现也可按设计解释。
