...
`ifdef DUMP_VCD
    initial begin
        $dumpfile("08_axi_cacheline_burst.vcd");
...
`endif
    ...
    axi_master dut (
        ...
        .dc_dev_wresp(dc_write_resp),
        .dc_dev_rrdy(dc_read_ready), .dc_cpu_ren(dc_read),
        .dc_cpu_raddr(dc_addr), .dc_cpu_uncached(dc_uncached),
        .dc_dev_rvalid(dc_valid),
        .dc_dev_rdata(dc_data),
    ...
    initial begin
        ...
        // Instruction read: aligned address and stable valid under pressure.
        @(posedge clk); ic_addr <= 32'h0000_0003; ic_req <= 1'b1;
        @(posedge clk); ic_req <= 1'b0;
        repeat (2) @(posedge clk);
        if (!arvalid || araddr != 32'h0000_0000 ||
            arlen != 3 || arsize != 3'b010)
        ...
        // A data read must win over a simultaneous instruction request.
        @(posedge clk); dc_addr <= 32'h0000_0012; dc_read <= 1'b1;
                        ic_addr <= 32'h0000_0040; ic_req <= 1'b1;
        @(posedge clk); dc_read <= 1'b0; ic_req <= 1'b0;
        #1;
        if (araddr != 32'h0000_0010)
            fail("data-read priority/alignment");
        arready <= 1'b1;
        @(posedge clk); arready <= 1'b0;
        send_rbeat(32'ha5a5_5a5a, 1'b1);
        #1;
        if (!dc_valid || dc_data[31:0] != 32'ha5a5_5a5a ||
            arlen != 0)
            fail("data R response");

        // AW and W can complete independently; WSTRB must remain stable.
        @(posedge clk);
        dc_waddr <= 32'h0000_0023;
        dc_wdata <= 32'hdead_beef;
        dc_wen <= 4'b1000;
        @(posedge clk); dc_wen <= 4'h0;
        #1;
        if (!awvalid || !wvalid || awaddr != 32'h20 ||
            wstrb != 4'b1000 || !wlast)
            fail("write launch");
        awready <= 1'b1;
        ...
        #1;
        if (awvalid || !wvalid)
            fail("independent AW handshake");
        repeat (2) @(posedge clk);
        if (!wvalid || wdata != 32'hdead_beef || wstrb != 4'b1000)
            fail("W channel changed under backpressure");
        wready <= 1'b1;
