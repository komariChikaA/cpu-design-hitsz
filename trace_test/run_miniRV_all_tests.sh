...
# Run every miniRV Trace test found in cdp-tests/bin and generate one report.
# The test list is discovered at runtime, so it includes the template, A-group,
# B-group, start, and any future .bin test added by the course framework.
#
# Usage:
#   ./run_miniRV_all_tests.sh [cdp-tests-directory] [report-file]
#
# Examples:
#   ./run_miniRV_all_tests.sh ~/cdp-tests
#   ./run_miniRV_all_tests.sh ~/cdp-tests ~/miniRV_all_report.md
...
if [[ ! -d "$bin_dir" ]]; then
    printf 'Error: cannot find the test binary directory: %s\n' "$bin_dir" >&2
    exit 2
fi
...
while IFS= read -r test_file; do
    ...
    tests+=("${test_name%.bin}")
done < <(find "$bin_dir" -maxdepth 1 -type f -name '*.bin' -print | LC_ALL=C sort)

if [[ ${#tests[@]} -eq 0 ]]; then
    printf 'Error: no .bin tests found in %s\n' "$bin_dir" >&2
    exit 2
fi

report_file="${2:-$test_root/trace_test/miniRV_all_report.md}"
if [[ "$report_file" != /* ]]; then
...

{
    ...
    printf -- '- 测试目录：`%s`\n' "$test_root"
    printf -- '- 编译结果：**%s**\n' "$build_result"
    printf -- '- 项目结果：**%d/%d 通过，%d 失败**\n\n' \
        "$pass_count" "${#tests[@]}" "$fail_count"

    printf '## 结果汇总\n\n'
    printf '| 项目 | 结果 | make 退出码 | 日志 |\n'
    printf '|---|---:|---:|---|\n'
    for index in "${!tests[@]}"; do
        test_name="${tests[$index]}"
