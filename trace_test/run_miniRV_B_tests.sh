...
# Run all miniRV group-B Basic Trace tests and generate a Markdown report.
#
# Usage:
#   ./run_miniRV_B_tests.sh [cdp-tests-directory] [report-file]
#
# Examples:
#   ./run_miniRV_B_tests.sh
#   ./run_miniRV_B_tests.sh ~/cdp-tests
#   ./run_miniRV_B_tests.sh ~/cdp-tests ~/miniRV_B_report.md
...
report_file="${2:-$test_root/trace test/miniRV_B_report.md}"
if [[ "$report_file" != /* ]]; then
...
if make >"$build_log" 2>&1; then
    ...
    {
        printf '# miniRV B 组 Basic Trace 测试报告\n\n'
        printf -- '- 生成时间：`%s`\n' "$started_at"
        printf -- '- 测试目录：`%s`\n' "$test_root"
